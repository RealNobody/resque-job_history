    ...
    module JobHistory
      ...
      module JobViewer
        ...
        def latest_job(class_name)
          ...
          Resque::Plugins::JobHistory::JobViewer.job_details(job_id, class_name)
        end

        module_function :latest_job

        def running_job_list(class_name, page_num, page_size)
          page_num  ||= 1
          page_size ||= Resque::Plugins::JobHistory::JobViewer.PAGE_SIZE

          start = (page_num - 1) * page_size
          history_klass.redis.lrange(history_klass.job_history_running_job_list_key(class_name),
                                     start,
                                     start + page_size - 1)
        end
        ...
        def finished_job_list(class_name, page_num, page_size)
          page_num  ||= 1
          page_size ||= Resque::Plugins::JobHistory::JobViewer.PAGE_SIZE

          start = (page_num - 1) * page_size
          history_klass.redis.lrange(history_klass.job_history_finished_job_list_key(class_name),
                                     start,
                                     start + page_size - 1)
        end
        ...
        def job_summaries(sort_key, sort_order, page_num, page_size)
          page_num  ||= 1
          ...
          if sort_order == "desc"
            ...
            jobs
          end[page_start..page_start + page_size - 1] || []
        end

        module_function :job_summaries

        def running_jobs(class_name, page_num, page_size)
          running_job_list(class_name, page_num, page_size).map do |job_id|
        ...
        def finished_jobs(class_name, page_num, page_size)
          finished_job_list(class_name, page_num, page_size).map do |job_id|
            job_details(job_id, class_name)
          end
        end
        ...
        def job_class_summary(class_name)
          { class_name:          class_name,
            running_jobs:        num_running_jobs(class_name),
            finished_jobs:       num_finished_jobs(class_name),
            total_finished_jobs: total_finished_jobs(class_name),
            max_running_jobs:    max_jobs_running(class_name),
        ...
        def max_jobs_running(class_name)
          history_klass.redis.get(history_klass.max_jobs_running_key(class_name))
        end

        module_function :max_jobs_running

        def total_finished_jobs(class_name)
          history_klass.redis.get(history_klass.total_finished_jobs_key(class_name))
        end
        ...
        module_function :sorted_job_summaries

        def num_finished_jobs(class_name)
          history_klass.redis.llen(history_klass.job_history_finished_job_list_key(class_name))
        end

        module_function :num_finished_jobs

        def num_running_jobs(class_name)
          history_klass.redis.llen(history_klass.job_history_running_job_list_key(class_name))
        end
